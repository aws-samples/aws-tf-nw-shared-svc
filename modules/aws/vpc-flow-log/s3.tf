...
data "aws_iam_policy_document" "vpc_flow_log" {
  ...
  statement {
    ...
    resources = [
      aws_s3_bucket.vpc_flow_log[0].arn,
      "${aws_s3_bucket.vpc_flow_log[0].arn}/*"
    ]
    condition {
      test     = "Bool"
  ...
  statement {
    ...
    resources = local.hive_compatible_partitions ? [
      "${aws_s3_bucket.vpc_flow_log[0].arn}/${local.log_prefix}/AWSLogs/aws-account-id=${local.account_id}/*"
      ] : [
      "${aws_s3_bucket.vpc_flow_log[0].arn}/${local.log_prefix}/AWSLogs/${local.account_id}/*"
    ]
    condition {
      test     = "StringEquals"
...
data "aws_s3_bucket" "vpc_flow_log" {
  count = local.enable_flow_log && local.flow_log_destination_type == "s3" ? 1 : 0

  bucket = local.create_s3 ? aws_s3_bucket.vpc_flow_log[0].id : var.flow_log_specs.destination_name
